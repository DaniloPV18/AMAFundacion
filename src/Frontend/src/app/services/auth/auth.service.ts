...
export class AuthService {
  ...
  SendCodeToResetPassword(sendCodeRequest: any): Observable<any> {
    return this.http.post(
      environment.serverAma + 'api/Auth/SendCodeToResetPassword',
      sendCodeRequest
    );
  }
  login(authRequest: AuthRequest): Observable<AuthDTO> {
    return this.http.post<AuthDTO>(
      `${environment.serverAma}api/Auth`,
      authRequest
    );
  }
  ...
  ResetPassword(sendCodeRequest: any): Observable<any> {
    return this.http.post(
      `${environment.serverAma}api/Auth/ResetPassword`,
      sendCodeRequest
    );
  }
  ...
  isLoggedIn(): boolean {
    ...
    if (authToken && expirationDate) {
      ...
      const hasExpired =
        currentDateTime - expirationDateTime > 24 * 60 * 60 * 1000;
