...
namespace FundacionAMA.Application.Services.CatalogoApp
{
    internal class DonationTypeServiceApp : IDonationTypeServiceApp
    {
        ...
        // esto modifico 
        public Task<IOperationResult<DonationTypeDto>> GetByIdentification(string identification)
        {
            return _donationTypeService.GetByIdentification(identification);
        }

        public async Task<IOperationResult<int>> GetCount()
        {
            return await _donationTypeService.GetCount();
        }
